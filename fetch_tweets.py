...
def get_tweet_list(twapi, idlist, tweet_file):
    '''
    ...
    with open(tweet_file, mode = 'a') as tweets_file:
        for tweet in tweets:
            # english_tweets.write('%s,%s\n' % (tweet.id, str(tweet.text.encode('UTF-8'))))
            tweets_file.write('%s,%s\n' % (tweet.id,
                                              str(tweet.text.encode('UTF-8')).replace(',', ' ')))
        

def get_tweets_bulk(twapi, idfilepath, tweet_file):
    '''
    ...
    # process IDs from the file
    tweet_ids = list()
    with open(tweet_file, mode = 'w') as tweets_file:
        tweets_file.write('%s,%s\n' % ('TweetID', 'tweet_texts'))
    with open(idfilepath, 'rb') as idfile:
        for line in idfile:
            tweet_id = get_tweet_id(line)
            ...
            if len(tweet_ids) == batch_size:
                ...
                get_tweet_list(twapi, tweet_ids, tweet_file)
                tweet_ids = list()
    # process remainder
    if len(tweet_ids) > 0:
...
def main(args):
    ...
    # hydrate tweet IDs
    if bulk:
        get_tweets_bulk(api, idfile, tweet_file=tweet_file)
    else:
